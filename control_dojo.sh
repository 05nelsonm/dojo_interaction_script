...
# and make sure you have the remote machine's rsa pubkeys in your Dojo machine`s root
# user`s `/root/.ssh/authorized_keys` file. See the README.md `ssh` section for more info.
                        ...
                        if [ "$EXECUTE" = "yes" ]; then
                                ...
                                if [ "SSH_OPTION" = "yes" ]; then
                                        if [[ "$EXTERNAL_BITCOIND" = "yes" && "${LOG_MODULES[$LNUM]}" = "bitcoind" ]]; then
                                                $SSH_CMD "tail -f $BITCOIN_DIR/debug.log"
                                        else
        ...
        if [ "${DOJO_CMD[$NUM]}" = "bitcoin-cli" ]; then
                ...
                while true; do
                        ...
                        if [[ "$BITCOIN_CLI_CMD" = "X" || "$BITCOIN_CLI_CMD" = "x" ]]; then
                                ...
                                if [ "SSH_OPTION" = "yes" ]; then
                                        if [ "$EXTERNAL_BITCOIND" = "no" ]; then
                                                $SSH_CMD "$CMD1 && ./dojo.sh ${DOJO_CMD[$NUM]} $BITCOIN_CLI_CMD"
                                        else
